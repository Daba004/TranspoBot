document.addEventListener('DOMContentLoaded', () => {
    ...
    function addMessage(text, isUser = false, data = null, sql = null) {
        ...
        if (sql) {
            ...
            sqlDiv.className = "mt-3 bg-gray-900 border border-gray-700 rounded-xl overflow-hidden shadow-lg p-3";
            sqlDiv.innerHTML = `
                <div class="text-[10px] text-gray-400 font-bold mb-2 uppercase tracking-widest flex items-center justify-between">
                    <span>Requête SQL générée</span>
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4" />
                    </svg>
                </div>
                <code class="text-xs text-green-400 font-mono break-words whitespace-pre-wrap">${sql}</code>
            `;
            messageDiv.querySelector('.space-y-3').appendChild(sqlDiv);
        }
    ...
    async function sendMessage(message) {
        ...
        try {
            ...
            if (result.success) {
                if (result.reponse) {
                    addMessage(result.reponse, false, result.data, result.sql);
                } else if (result.data && result.data.length > 0) {
                    addMessage(`J'ai trouvé ${result.data.length} résultat(s) pour votre demande :`, false, result.data, result.sql);
                } else if (result.data && result.data.length === 0) {
                    addMessage("Aucun résultat ne correspond à votre recherche dans la base de données.", false, null, result.sql);
                } else {
                    addMessage("C'est fait ! La requête SQL a été exécutée avec succès.", false, null, result.sql);
                }
            } else {
