...
<div class="bg-white rounded-2xl border border-slate-200/60 overflow-hidden shadow-lg shadow-slate-200/30 flex-1 flex flex-col min-h-0">
    <div class="p-6 border-b border-slate-100 flex justify-between items-center bg-white/50 backdrop-blur-sm shrink-0">
        ...
        <div class="flex flex-wrap gap-2">
            <button onclick="addTrajet()" class="bg-amber-400 text-emerald-950 px-4 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-amber-300 hover:shadow-lg transition-all flex items-center gap-2">
                ...
                <tr class="group hover:bg-emerald-50/40 transition-all duration-300">
                    ...
                    <td class="px-6 py-4 text-right whitespace-nowrap space-x-1">
                        <button onclick='showDetails(<?php echo json_encode($t); ?>, <?php echo json_encode($tarifs_by_ligne[$t['ligne_id']] ?? []); ?>)' 
                                class="bg-white text-slate-800 px-3 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-emerald-700 hover:text-white hover:shadow-lg transition-all border border-slate-200 group-hover:border-emerald-700">
                            Détails
                        </button>
                        <button onclick="deleteTrajet(<?php echo $t['id']; ?>)"
                                class="bg-white text-rose-600 px-3 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-rose-600 hover:text-white hover:shadow-lg transition-all border border-rose-200">
                            Sup.
                        </button>
                    </td>
                </tr>
...
async function deleteTrajet(id) {
    ...
    try {
        const res = await fetch(`${window.location.origin}/api/trajets/${id}`, { method: 'DELETE' });
        ...
        alert("Erreur lors de la suppression. Le backend Python est-il lancé ?");
    }
}
</script>
